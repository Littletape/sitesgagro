 $(document).ready(function(){
            ...
            $(document).on('click','#salvar',function(){
                ...
                env.cargo = $('#cargo').val();
                env.area = $('#area  option:selected').text();
                env.idarea = $('#area  option:selected').attr('idArea');
               ...
               if ($('#autor').is(':checked') && $('#avaliador').is(':checked') == false ){
                ...
                if($('#autor').is(':checked') == false && $('#avaliador').is(':checked') == false){ 
                    modalAlerta('Você não selecionou nehuma opção no campo cadastrar como.');
                    env.tipocadastro = '';
                    return;
                ...
                if (count != 0){
                    modalAlerta('Atenção aos campos com preenchimento obrigatórios!!!');
                 return;
                // senão submete    
                }
                ...
                if(anos < 17 || anos > 90){
                modalAlerta("Data de nascimento invalida, verifique e tente novamente");
                return;
                }

                    // verifica se a confirmação de senha e mail correspondem
                    if(env.senha != env.confirmar_senha & env.email != env.confirmar_email){
                       modalAlerta('O campo de confirmação da senha e do email não correspondem aos mesmos, verifique e tente novamente');
                    ...
                    if(env.senha != env.confirmar_senha){
                       modalAlerta('O campo de confirmação da senha não corresponde a mesma, verifique e tente novamente');
                       $('#senha').css({"border" : "2px inset #F00", "padding": "2px"});
                       $('#confirmar_senha').css({"border" : "2px inset #F00", "padding": "2px"});
                        return;
                    }else{
                    ...
                    if(env.email != env.confirmar_email){
                       modalAlerta('O campo de confirmação do email não correspondem ao mesmo, verifique e tente novamente');
                       $('#email').css({"border" : "2px inset #F00", "padding": "2px"});
                       $('#confirmar_email').css({"border" : "2px inset #F00", "padding": "2px"});
                       return;
                    }else{
